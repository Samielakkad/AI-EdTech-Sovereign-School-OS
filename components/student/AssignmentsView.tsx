...
import { StudentProfile, Assignment, AssignmentSubmission } from '../../types.ts';
import Button from '../common/Button.tsx';
import * as studentDataService from '../../services/studentDataService.ts';
// FIX: Moved UploadCloudIcon from SettingsIcon to StudentIcons import.
import { ClipboardCheckIcon, CheckCircleIcon, UploadCloudIcon } from '../icons/StudentIcons.tsx';
...
const SubmissionModal: React.FC<{
    ...
    // The dueDate string is 'YYYY-MM-DD'. Appending T00:00:00 makes it parse as local time instead of UTC.
...
const AssignmentsView: React.FC<AssignmentsViewProps> = ({ student, onUpdate }) => {
    const [selectedAssignment, setSelectedAssignment] = useState<Assignment | null>(null);

    const handleSubmit = (submission: Omit<AssignmentSubmission, 'submittedDate'>) => {
    ...
    const submittedAssignments = student.assignments
        .filter(a => a.status !== 'pending')
        .sort((a, b) => new Date(b.submission?.submittedDate || b.assignedDate).getTime() - new Date(a.submission?.submittedDate || a.assignedDate).getTime());
        ...
        <>
            <div className="bg-gray-800 rounded-lg shadow-lg p-6 h-full">
                ...
                <div className="space-y-6">
                     <div>
                        <h3 className="text-lg font-bold text-gray-300 mb-3">Upcoming ({upcomingAssignments.length})</h3>
                    ...
                    {submittedAssignments.length > 0 && (
                        <div className="pt-4 mt-4 border-t border-gray-700">
                             <h3 className="text-lg font-bold text-gray-400 mb-3">Submitted & Graded ({submittedAssignments.length})</h3>
                             <div className="space-y-3">
