...
const StudentRow: React.FC<{ student: StudentProfile, onSelect: (id: string) => void; onContact: (id: string) => void; }> = ({ student, onSelect, onContact }) => {
    ...
    return (
        <div className="group flex items-center justify-between bg-gray-700/50 hover:bg-gray-700 p-3 rounded-md transition-colors text-left">
            ...
            >
                {student.avatarUrl ? (
                    <img src={student.avatarUrl} alt={student.name} className="w-8 h-8 rounded-full mr-3 object-cover" />
                ) : (
                ...
                {student.status !== 'absent' && (
                    <>
                        <div className="text-right">
                             <div className="font-bold text-sm text-white">{student.attendancePercentage}%</div>
                 ...
                 <button 
                    ...
                    className="opacity-0 group-hover:opacity-100 transition-opacity p-2 rounded-full hover:bg-indigo-600"
