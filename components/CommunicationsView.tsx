...
import { Conversation, Message, Parent, CommunicationTopic, communicationTopicLabels } from '../types.ts';
...
const ComposeModal: React.FC<ComposeModalProps> = ({ isOpen, onClose, onSend, mode, parents, conversation, initialData }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (mode === 'reply' && conversation) {
                ...
                // Try to infer topic from subject
    ...
    const handleGenerateDraft = async () => {
        ...
        const draft = await geminiService.draftFormalResponse(topic, keyPoints, parent.studentName, parent.name, mockTeacher.name);
...
const ParentCommunicationCenter: React.FC<ParentCommunicationCenterProps> = ({ targetStudentId, suggestedTopic, onTargetHandled }) => {
