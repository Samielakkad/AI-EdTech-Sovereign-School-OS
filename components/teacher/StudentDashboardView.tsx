...
import { StudentProfile, IncidentSeverity, IncidentType, CommunicationTopic } from '../../types.ts';
...
import { MailIcon } from '../icons/SettingsIcon.tsx';
...
const StudentDashboardView: React.FC<StudentDashboardViewProps> = ({ studentId, onBack, onContactParent }) => {
    ...
    const hasRecentConcern = student.incidents.some(inc => 
        ...
        (new Date(inc.timestamp) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000))
    );
