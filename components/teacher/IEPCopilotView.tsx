...
import { IEPGoal, StudentProfile } from '../../types.ts';
...
const IEPCopilotView: React.FC = () => {
    ...
    const fetchAllData = () => {
        const studentProfiles = studentDataService.getStudentProfiles();
        setStudents(studentProfiles);
        if (studentProfiles.length > 0 && !studentId) {
            setStudentId(studentProfiles[0].id);
        }

        const allGoals = studentProfiles.flatMap(p => p.iepGoals.map(g => ({ ...g, studentName: p.name })));
                ...
                <div className="lg:col-span-1 flex flex-col gap-6">
                    <div className="bg-gray-800 rounded-lg shadow-lg p-6">
                     ...
                     <div className="bg-gray-800 rounded-lg shadow-lg p-6 flex flex-col">
                        ...
                        {inferredFocusArea && !isLoading && (
                            <div className="mb-4 bg-gray-900/50 p-3 rounded-md border border-gray-700">
                                <p className="text-xs font-semibold text-gray-400">AI-Identified Focus Area:</p>
                                <p className="font-semibold text-indigo-300">{inferredFocusArea}</p>
                            </div>
                        )}
                        <div className="flex-1 mb-4">
                            {isLoading ? (
                                ...
                                <textarea
                                    className="w-full h-full bg-gray-700/50 rounded-md p-4 text-gray-300 whitespace-pre-wrap font-mono resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                    value={editableGoal}
                        ...
                        {!isLoading && generatedGoal && (
                            <Button onClick={handleSaveGoal} disabled={isSaved || !editableGoal} className="w-full">
